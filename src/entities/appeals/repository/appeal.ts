...
import { AppealCanceled, AppealWithRelations, AppealResolved, AppealPending, AppealCreated } from "../domain";
        ...
        switch (appeal.status) {
            ...
            case "CANCELED": {
                const [update] = await prisma.$transaction([
                    ...
                    ...appeal.cancellation ? [prisma.appealCancellation.delete({
                        where: { appealId: id }
                    })] : []
                ]);
            ...
            case "RESOLVED": {
                const [update] = await prisma.$transaction([
                    ...
                    ...appeal.resolution ? [prisma.appealResolution.delete({
                        where: { appealId: id }
                    })] : []
                ]);
    ...
    try {
        ...
        if (appeal.status === "NEW" || appeal.status === "PENDING"){
            const [updateData, createData] = await prisma.$transaction([
                ...
                ...(reason ? [prisma.appealCancellation.create({
            ...
            const elseText = 'Only appeals with statuses NEW or PENDING can be changed to CANCELED status.' 
...
const cancelAll = async function() {
    try {
        const updateRes = await prisma.appeal.updateManyAndReturn({
            where: {
                OR:
                [
                    {status: "PENDING"}, 
        ...
        }).then(appeals => appeals.map(appeal => ({ ...appeal, cancellation: null } as AppealCanceled)))
    ...
    try {
        ...
        if (appeal.status === "NEW" || appeal.status === "PENDING"){
            const [updateData, createData] = await prisma.$transaction([
                ...
                ...(text ? [prisma.appealResolution.create({
            ...
            const elseText = 'Only appeals with statuses NEW or PENDING can be changed to RESOLVED status.' 
            return createResultObj('error', elseText, "forbidden")
        }
    } catch (e) {
...
const findAll = async({datesQuery, resolution, cancellation}:
    ...
    try {
        ...
        }).then(appeals => appeals.map(appeal => cleanUnnecessaryAppealFields(appeal as AppealWithRelations))) as AppealWithRelations[]
